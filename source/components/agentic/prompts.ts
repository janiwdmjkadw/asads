/**
 * The prompt corpus for `/agentic-trading`.
 *
 * These are written in the voice our users actually use — run-on,
 * lowercase, ticker-and-address soup, a whole thesis in one breath. The
 * page's entire argument is that this sentence IS the strategy, so the
 * examples are the product demo; keep them real, never sanitised into
 * marketing copy.
 *
 * `tags` name the agent capabilities a prompt exercises and are display
 * chips only — nothing routes off them.
 */
...
/** Hero: the three that read fastest as "you can just say that". */
...
export const PROMPT_GALLERY: readonly AgentPrompt[] = [
  {
...
/** Split into two marquee rows of near-equal length. */
export const GALLERY_ROW_A: readonly AgentPrompt[] = PROMPT_GALLERY.slice(0, 7);
export const GALLERY_ROW_B: readonly AgentPrompt[] = PROMPT_GALLERY.slice(7);
