    ...
    <Dialog open={open} onOpenChange={(next) => (!next ? onClose() : undefined)}>
      <DialogContent
        ...
        style={{
          ...
          maxWidth: 'none',
          background: 'var(--panel-bg, #0d0d11)',
          ...
          borderRadius: 22,
          boxShadow:
            'inset 0 1px 0 rgba(255,255,255,0.06), 0 0 44px -18px color-mix(in srgb, var(--accent-primary) 65%, transparent), 0 24px 60px rgba(0,0,0,0.5)',
        }}
      >
    ...
    <div style={{ position: 'relative', padding: '18px 18px 16px' }}>
        ...
        >
          <span
            style={{
              ...
              fontWeight: 700,
              color: 'var(--ink-0)',
              border: '2px solid var(--panel-bg, #0d0d11)',
      ...
      <div
        ...
        style={{
          ...
          background: `radial-gradient(140% 70% at 50% 0%, color-mix(in srgb, ${identity.main} 4%, transparent) 0%, transparent 48%), rgba(0,0,0,0.35)`,
      ...
      {[...markers.values()].map(({ index, side }) => {
        ...
        return (
          <g key={`${index}:${side}`}>
            ...
            <circle cx={cx} cy={cy} r={7} fill={fill} stroke="rgba(0,0,0,0.55)" strokeWidth={1.5} />
            <text
              x={cx}
              y={cy + 3.2}
              textAnchor="middle"
...
function McStat({
  ...
  return (
    <div style={{ background: 'var(--panel-bg, #0d0d11)', padding: '11px 10px', textAlign: 'center' }}>
      <div
        style={{
          display: 'inline-flex',
          alignItems: 'center',
