import './art.css';
import { feeAuthority, sol, type FeeAuthority } from '@/components/discover/column/rowData';
...
export function FeeVariants() {
  /*
   * The three states the panel has to survive: one recipient at 100%, a
   * two way split, and an UNLOCKED authority. The last is the state it
   * exists for and the one a mock never shows.
   */
  const states: readonly [string, FeeAuthority][] = [
    ['One recipient', feeAuthority(3, true)],
    ['Split', feeAuthority(1, false)],
    ['Unlocked', feeAuthority(2, false)],
  ];
  return (
    <div className="fp-sheet">
      {states.map(([label, auth]) => (
        <div className="fp-col" key={label}>
